<template>
    <main>
        ...
        <div class="lg:lg:w-[500px] p-4 p-4  flex flex-col justify-start items-start">
         <div class="flex justify-start items-start">
          ...
          <form class="w-full space-y-6" @submit.prevent="register">
            <div class="mb-4">
              <label class="block text-[#7D8799] font-medium mb-1 text-sm" for="phone">Enter Your Username</label>
              <input type="text" id="username" v-model="credential.username.value" class="w-full px-4 py-4  bg-[#F4F5F7] outline-none border-[0.5px] border-[#F4F5F7] rounded-md focus:outline-none focus:border-green-500" />
            </div>
            <div class="mb-4">
            ...
            <div class="mb-4 relative">
              <label class="block text-[#7D8799] font-medium mb-1 text-sm" for="passcode">Enter Your password</label>
            ...
            <div class="pt-6">
              <button :disabled="loading || isFormDisabled" type="submit" class="w-full bg-[#2F6D67] text-white py-3.5 disabled:cursor-not-allowed disabled:opacity-25 rounded-md hover:bg-[#2F6D67] transition">{{loading ? 'processing...' :  'Signup'}}</button>
            </div>
          </form>
      ...
      <FullScreenLoade :visible="loading" text="Please wait while we create your account" />
    </main>
      </template>
